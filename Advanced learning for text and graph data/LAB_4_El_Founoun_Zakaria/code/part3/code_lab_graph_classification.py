...
def graphlet_kernel(Gs_train, Gs_test, n_samples=200):
    ...
    for i, G in enumerate(Gs_train):
        ...
        for k in range(n_samples):
            n_=np.random.choice(nodes_train,size=3,replace=False)
            sub_g= G.subgraph(n_)
    ...
    for i, G in enumerate(Gs_test) :
        nodes_test= G.nodes()
        for k in range(n_samples):
            n_=np.random.choice(nodes_test,size=3,replace=False)
            sub_g=  G.subgraph(n_)
            for j in range(len(graphlets)):
                if nx.is_isomorphic(graphlets[j], sub_g):
                    phi_test[i,j]+=1
...
print("The accuracy of shortest_path_kernel method : ", accuracy_score(y_test, y_pred_sp))

# Using graphlet kernel

clf_gl=SVC(kernel='precomputed')
clf_gl.fit(K_train_gl, y_train)
y_pred_gl = clf_gl.predict(K_test_gl)
print("The accuracy of graphlet_kernel method : ", accuracy_score(y_test, y_pred_gl))
